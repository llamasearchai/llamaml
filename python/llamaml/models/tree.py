...
class DecisionTree(Model):
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.criterion = config.params.get(
            "criterion", "gini" if self.task == "classification" else "mse"
        )

        if self.task not in ["classification", "regression"]:
            raise ValueError(
                f"Task must be 'classification' or 'regression', got {self.task}"
            )
    ...
    def _find_best_split(
        ...
        if self.task == "classification":
            current_impurity = (
                self._gini_impurity(y) if self.criterion == "gini" else self._entropy(y)
            )
        else:
...
class RandomForest(Model):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "RandomForest":
        ...
        if self.max_features == "sqrt":
            ...
            isinstance(self.max_features, int) and 1 <= self.max_features <= n_features
        ):
